...
Consolidated status for `ADR-001` through `ADR-013`, per
...
|---|---|---|---|---|
| ADR-001 | App identity and migration | Accepted; group migration superseded by ADR-013 | Worker Bee (engineering default, pending product owner override) | P1-T02, P2-T02, P6-T05 |
...
| ADR-004 | Conversation identity | Superseded by ADR-013 | Worker Bee | none |
| ADR-005 | Speech queue policy | Accepted; group settings superseded by ADR-013 | Worker Bee | P4-T03 |
...
| ADR-009 | Privacy, storage, logging | Accepted; group storage superseded by ADR-013 | Worker Bee | P2-T02, P6-T03 |
...
| ADR-013 | Group-reader removal | Accepted | OpenCode | Current production behavior |

ADR-004 and the group-specific portions of ADR-005 are historical and superseded
by ADR-013. No ADR is deferred. Per plan rules, this means
...
- ADR-013 supersedes the historical three-mode group policy. Upgrades discard
  all group policy and selection state rather than guessing intent.
